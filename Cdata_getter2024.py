'''
#TDL
Wingspans for all players/part names issue ie 'davis bert'
...
'''
class DataGetter:
    def __init__(self,execution='n/a',p_inf_meta=True,manual_pull=False,season='2024-25'):
            if execution == 'normal_run':
                self.date = time.strftime('%m-%d-%Y')
                self.script_directory = os.path.dirname(os.path.abspath(__file__))
                
                self.get_data(player_info_meta = p_inf_meta, manual_pull=manual_pull)
                #METADATA
                
                self.players = self.read_players()
                self.self_players_cleaner()
                print(self.players)
            ...
            if execution == 'run1':                
                ...
                self.game_ids = self.get_gameids()	#collects and manages the game_ids of the current season
                #self.rosters = self.read_rosters()
                self.teams = self.read_teams()
                #self.players = self.read_players()
                ...
                #GAMELOGS
                self.get_data(player_info_meta = p_inf_meta, manual_pull=manual_pull) #fetches original gamelogs and saves to dir || set player_info_meta to be True for each season
                ...
                #Update Player Game Logs (with what?)
    ...
    def normalize_player_name(self, name):
        replacements = {'ā': 'a', 'ć': 'c', '.': '', ' Jr': '', ' Jr.': '','č':'c',
                        ' Sr':'','Š':'S','ģ':'g','ū':'u','ö':'o','š':'s','é':'e','ņ':'n',
                        'ü': 'u', 'Č': 'C', 'ñ': 'n','Ş':'S','é':'e','ö':'o','í':'i','ô':'o'}
        for src, target in replacements.items():
            name = name.replace(src, target)
        return name.strip()

    def match_player_name(self, name):
        #This method returns a player name for the players in self.players
        #must be able to handle different carachter encodings
        #might have to manually update for new players
    ...
    def scrape_and_update_players(self, year=2025):
    ...
    def normalize_player_names(self):
        ...
        for name, attributes in self.players.items():
            # Normalize the name to ASCII, ignoring non-ASCII characters
            normalized_name = unicodedata.normalize('NFKD', name).encode('ASCII', 'ignore').decode('ASCII')
            normalized_name =  self.normalize_player_name(normalized_name)
            
            if normalized_name in normalized_players:
                # If there's a duplicate, decide which one to keep (here we keep the first occurrence)
                print(f"Warning: Duplicate name found after normalization: {normalized_name}. Keeping the first occurrence.")
            else:
                normalized_players[normalized_name] = attributes
        ...
        if response.status_code == 200:
            ...
            for row in rows:
                ...
                if name_td and wingspan_td:
                    ...
                    player_name = player_name_org[:-3] #cuts off some proper names
                    #print(player_name) #this should be match with a player name in self.players
                    ...
                    if matched_name:
                        if 'bio' not in self.players[matched_name]:
                            self.players[matched_name]['bio'] = {}
                        self.players[matched_name]['bio']['WINGSPAN'] = wingspan
                    else:
                        try:
                            if player_name_org == 'Jakob Pöltl':
                                player_name_org = 'Jakob Poltl'
                            if 'bio' not in self.players[player_name_org]:
                                self.players[player_name_org]['bio'] = {}
                            self.players[player_name_org]['bio']['WINGSPAN'] = wingspan
                        except KeyError:
                            print(f'Modified: {player_name} plyr_nm_org: {player_name_org} addition failed for wingspan')
        else:
    ...
    def process_advanced_stats(self, year):
        ...
        if response.status_code == 200:
            soup = BeautifulSoup(response.content, 'html.parser')
            players = soup.select('#advanced tbody tr[class!="thead"]')
            for player in players:
                player_name = self.normalize_player_name(player.select_one('[data-stat="name_display"]').text)
                if player_name == 'League Average':
                    continue
                ...
                try:
                    if matched_name:
                        self.players[matched_name]['advanced'] = {}
                        self.players[matched_name]['advanced'].update({'OBPM': obpm, 'DBPM': dbpm})
                    else:
                        print(f"Name mismatch or not found in players: {player_name}")
                except KeyError as e:
                    print(f'Player: {matched_name} is not in self.players')
    ...
    def process_play_by_play(self, year):
        ...
        if response.status_code == 200:
            ...
            for player in players:
                #'player': the html block for a given player in the bref leaderboards
                player_name = self.normalize_player_name(player.select_one('[data-stat="name_display"]').text)
                ...
                g = int(player.select_one('[data-stat="games"]').text)
                ...
                try:
                    if matched_name:
                        self.players[matched_name]['pbp'] = {}
                        self.players[matched_name]['pbp'].update({'and1_total':and1,'shotFoulsCommited_total':shooting_fouls,'shotFoulsDrawn_total':shooting_fouls_drawn,'shotFoulsCommited_min': shootingFouls_min,'ptsGeneratedByAst_min':ptsGenAst_min})
                    else:
                        print(f"Name mismatch or not found in players: {player_name}")
                except KeyError as e:
                    print(f'Player: {matched_name} is not in self.players')
    ...
    def self_players_cleaner(self):
        if 'bio' in self.players.keys():
            del self.players['bio']
    ...
    def read_rosters(self):
        ...
        for player_name, player_info in self.players.items():
            try:
                ...
                team_abbreviation = player_info['bio']['TEAM_ABBREVIATION']
                team_id = player_info['bio']['TEAM_ID']
                position = player_info['bio']['POSITION']
    ...
    def build_player_dict(self, manual_nbaapi_pull=False):
        directory = 'games/2024-25/'
        player_dict = {}
        print('building player dict')
        #player dict builds out from gamelogs that exist already
        #need to get current season gamelogs for players
        
        
        if manual_nbaapi_pull == True:
            #access nba player json for current season
            #probably through object in one if the imports obove
            #will look like this for example, last years players dict {"Scottie Barnes": {"id": 1630567, "HEIGHT": 200.66, "WEIGHT": 237.0, "BIRTHDATE": "2001-08-01", "SEASON_EXP": "3", "TEAM_ID": "1610612761", "TEAM_ABBREVIATION": "TOR", "POSITION": "Forward-Guard", "ROSTERSTATUS": "Active"}, "Bruce Brown": {"id": 1628971, "HEIGHT": 193.04, "WEIGHT": 202.0, "BIRTHDATE": "1996-08-15", "SEASON_EXP": "6", "TEAM_ID": "1610612761", "TEAM_ABBREVIATION": "TOR", "POSITION": "Guard-Forward", "ROSTERSTATUS": "Active"}, "Thaddeus Young": {"id": 201152, "HEIGHT": 203.2, "WEIGHT": 225.0, "BIRTHDATE": "1988-06-21", "SEASON_EXP": "18", "TEAM_ID": "1610612756", "TEAM_ABBREVIATION": "PHX", "POSITION": "Forward", "ROSTERSTATUS": "Inactive"}, "Gary Trent Jr": {"id": 1629018, "HEIGHT": 195.58, "WEIGHT": 204.0, "BIRTHDATE": "1999-01-18", "SEASON_EXP": "6", "TEAM_ID": "1610612749", "TEAM_ABBREVIATION": "MIL", "POSITION": "Guard", "ROSTERSTATUS": "Active"}, "Dennis Schroder": {"id": 203471, "HEIGHT": 185.42, "WEIGHT": 175.0, "BIRTHDATE": "1993-09-15", "SEASON_EXP": "11", "TEAM_ID": "1610612751", "TEAM_ABBREVIATION": "BKN", "POSITION": "Guard", "ROSTERSTATUS": "Active"}, "Gradey Dick": {"id": 1641711, "HEIGHT": 198.12, "WEIGHT": 200.0, "BIRTHDATE": "2003-11-20", "SEASON_EXP": "1", "TEAM_ID": "1610612761", "TEAM_ABBREVIATION": "TOR", "POSITION": "Guard-Forward", "ROSTERSTATUS": "Active"}, "Jordan Nwora": {"id": 1629670, "HEIGHT": 203.2, "WEIGHT": 225.0, "BIRTHDATE": "1998-09-09", "SEASON_EXP": "5", "TEAM_ID": "1610612761", "TEAM_ABBREVIATION": "TOR", "POSITION": "Forward", "ROSTERSTATUS": "Inactive"}, "Jontay Porter": {"id": 1629007, "HEIGHT": 208.28, "WEIGHT": 240.0, "BIRTHDATE": "1999-11-15", "SEASON_EXP": "2", "TEAM_ID": "0", "TEAM_ABBREVIATION": "", "POSITION": "Center-Forward", "ROSTERSTATUS": "Inactive"}, "Chris Boucher": {"id": 1628449, "HEIGHT": 205.74, "WEIGHT": 200.0, "BIRTHDATE": "1993-01-11", "SEASON_EXP": "7", "TEAM_ID": "1610612761", "TEAM_ABBREVIATION": "TOR", "POSITION": "Forward", "ROSTERSTATUS": "A
            from nba_api.stats.static import players

            # Get all NBA players
            all_players = players.get_players()
            all_players = [player for player in all_players if player['is_active'] == True]
            # Create a dictionary where the key is the full name and value is a dict with player details
            player_dict = {player['full_name']: {'id': player['id']} 
                            for player in all_players}
            print(player_dict)
            
        if manual_nbaapi_pull == False:    
    ...
    def player_meta(self, build_player_dict=True,manual_pull=False):
        print('player meta')
        if build_player_dict:  # only when updaing player_info json
            self.players = self.build_player_dict(manual_nbaapi_pull=manual_pull)
        else:
            self.players = self.read_players()
        ...
        print('This is the original self.players dict after creating a player and his ID')
        print(self.players)
    ...
    def get_data(self, player_info_meta = False, manual_pull=False):
        if player_info_meta == True:
            self.players = self.player_meta(manual_pull=manual_pull)
        else:
            self.players = self.read_players()
        ...
        print('This is the starting self.players dict: ', self.players)
        if 'bio' in self.players.keys():
            del self.players['bio']
        for player, info in self.players.items():
            player_id = info['id']
            time.sleep(0.5)  # Be cautious with using time.sleep in production code
            result = self.fetch_player_game_logs(player_id, '2024-25')
            ...
            if isinstance(result, pd.DataFrame):
                ...
                print(f"Error fetching data for {player} result variable is not panda dataframe: {result}")
        self.scrape_and_update_players()
    ...
    def append_averages_todict(self): #Appends player attributes to themselves
        ...
        for player, attr in self.players.items():
            try:
                ...
                self.players[player]['gamelogs'] = {'offence':{},'defence':{}}
                self.players[player]['gamelogs']['AVG_MIN'] = round(df['MIN'].mean(), 2)
                self.players[player]['gamelogs']['AVG_PACE'] = round(df['pace'].mean(), 2)
                self.players[player]['gamelogs']['AVG_SPEED'] = round(df['speed'].mean(), 2)
                self.players[player]['gamelogs']['AVG_DISTANCE'] = round(df['distance'].mean(), 2)
                self.players[player]['gamelogs']['AVG_POSSESSIONS'] = round(df['possessions'].mean(), 2)
                self.players[player]['gamelogs']['AVG_REB_PCT'] = round(df['reboundPercentage'].mean(), 2)
                self.players[player]['gamelogs']['AVG_REB_CHANCES'] = round(df['reboundChancesTotal'].mean(), 2)
                self.players[player]['gamelogs']['AVG_REB'] = round(df['REB'].mean(), 2)
                
                self.players[player]['gamelogs']['POSS_REB_CHANCES'] = round((df['reboundChancesTotal'] / df['possessions']).mean(), 2)
                self.players[player]['gamelogs']['offence']['OFF_AVG_PTS'] = round(df['PTS'].mean(), 2)
                self.players[player]['gamelogs']['offence']['OFF_AVG_AST'] = round(df['AST'].mean(), 2)
                self.players[player]['gamelogs']['offence']['OFF_AVG_FGA'] = round(df['FGA'].mean(), 2)
                self.players[player]['gamelogs']['offence']['OFF_AVG_3PA'] = round(df['FG3A'].mean(), 2)
                self.players[player]['gamelogs']['offence']['OFF_AVG_OREB_PCT'] = round(df['offensiveReboundPercentage'].mean(), 2)
                self.players[player]['gamelogs']['offence']['OFF_AVG_PTS_PAINT'] = round(df['pointsPaint'].mean(), 2)
                self.players[player]['gamelogs']['offence']['OFF_AVG_OFF_RATING'] = round(df['offensiveRating'].mean(), 2)
                self.players[player]['gamelogs']['offence']['OFF_AVG_USG_PCT'] = round(df['estimatedUsagePercentage'].mean(), 2)
                self.players[player]['gamelogs']['offence']['OFF_AVG_TOUCHES'] = round(df['touches'].mean(), 2)
                self.players[player]['gamelogs']['offence']['OFF_AVG_PASSES'] = round(df['passes'].mean(), 2)
                self.players[player]['gamelogs']['offence']['OFF_AVG_TOV'] = round(df['TOV'].mean(), 2)
                
                self.players[player]['gamelogs']['offence']['OFF_POSS_PTS'] = round((df['PTS'] / df['possessions']).mean(), 2)
                self.players[player]['gamelogs']['offence']['OFF_POSS_AST'] = round((df['AST'] / df['possessions']).mean(), 2)
                self.players[player]['gamelogs']['offence']['OFF_POSS_REB'] = round((df['REB'] / df['possessions']).mean(), 2)
              
                self.players[player]['gamelogs']['defence']['DEF_AVG_PTS'] = round(df['oppPoints'].mean(),2)
                def_pts_poss = round((df['oppPoints'] / df['possessions']).mean(),2)
                if np.isfinite(def_pts_poss):
                    self.players[player]['gamelogs']['defence']['DEF_POSS_PTS'] = def_pts_poss
                else:
                    self.players[player]['gamelogs']['defence']['DEF_POSS_PTS'] = 'n/a'
                self.players[player]['gamelogs']['defence']['DEF_AVG_AST'] = round((df['matchupAssists'] / df['possessions']).mean(),2)
                ...
                print(player,'Key Error..Removed', e)
    ...
    def get_team_standings(self):
        standings = leaguestandings.LeagueStandings(season=yr).get_data_frames()[0]
    ...
    def get_team_metrics(self):
        team_metrics = teamestimatedmetrics.TeamEstimatedMetrics(season=yr).get_data_frames()[0]
    ...
    def combine_team_dfs(self):
        ...
        print(team_metrics,team_metrics.columns,team_standings,team_standings.columns)
    ...
    def merge_teamplayer_data(self):
        ...
        #Do the same thing but for team gamelogs as well
        #
        for filename in os.listdir('players/gamelogs/'):
            ...
            if '._' or '_.' not in filepath:
                try:
                    ...
                    try:
                        ...
                        player_log.to_csv(file_path, index=False)
                        print(f'Key Error {e} + option 2 complete')
    ...
    def merge_team_game_data(self):
        # Load the team metrics data with z-scores
        team_metrics = pd.read_csv('teams/metadata/team_data_zscore.csv')

        # Abbreviation mapping to ensure the teams match correctly
        team_metrics = self.abbreviation(team_metrics)

        # Iterate over each team gamelog file
        for filename in os.listdir('teams/games/gamelogs/'):
            file_path = os.path.join('teams/games/gamelogs/', filename)
            
            if '._' not in file_path and '_.' not in file_path:
                try:
                    team_log = pd.read_csv(file_path)

                    
                    # Ensure 'Opponent' is mapped correctly
                    for index, row in team_log.iterrows():
                        opponent_abbrev = row['TEAM_NAME']  # Assuming 'TEAM_ABBREVIATION' column is present
                        
                        # Fetch corresponding team metrics using abbreviation
                        team_data = team_metrics[team_metrics['TEAM_NAME'] == opponent_abbrev]
                        
                        # Update the team log with z-score metrics for the opponent
                        for col in ['W_PCT', 'E_OFF_RATING', 'E_DEF_RATING', 'E_NET_RATING', 'E_PACE', 'E_REB_PCT']:
                            if not team_data.empty:
                                team_log.at[index, col] = team_data.iloc[0][col]

                    # Save the updated gamelog
                    team_log.to_csv(file_path, index=False)

                except UnicodeDecodeError:
                    print(f"Unicode error in file: {filename}")
                    continue
        ...
        print('Team Game Log and Metric Merger complete.')
    ...
    def clean_df(self, df, t):
        ...
        if t == 'player_df':
            for index, row in dataframe.iterrows():
                ...
                if player_info == {}:
                    continue
                dataframe.loc[index, 'Player Height'] = player_info['bio'].get('HEIGHT', 0)
                dataframe.loc[index, 'Player Weight'] = player_info['bio'].get('WEIGHT', 0)
                dataframe.loc[index, 'True Position'] = player_info['bio'].get('POSITION', '0')
                    
        return dataframe
                
        
    def accessNBA_API_boxscores(self):
        file_path = 'games/2024-25'
        retry_count = 3
    ...
    def add_boxscoregamelog(self):
        #For each game ina  players gamelog, this method matches the game to the games boxscore and appends extra boxscore data fro each game
        #if you get a duplicate column error, the problem lies within the player gmelogs sctruct for certain players
        ...
        for filename in os.listdir('players/gamelogs/'):
            ...
            if '._' or '_.' not in filepath:
                try:
                    all_rows = []
                    player_log = pd.read_csv(file_path) #df1
                    ...
                    for index, row in player_log.iterrows():
                        ...
                        game_path = f'games/2024-25/{gameid}/player_BoxScores.csv'
                        if os.path.exists(game_path):
                            game = pd.read_csv(game_path) #Unique game boxscore
        ...
        for filename in os.listdir('teams/games/gamelogs/'):
            ...
            if '._' or '_.' not in filepath:
                try:
                    all_rows = []
                    team_log = pd.read_csv(file_path)
                    #team log: LAC.csv for example
                    #why does it contain last years games instad of this years? the season is set to 2024-25?
                    for index, row in team_log.iterrows():
                        gameid = row['Game_ID']
                        gameid = f'00{gameid}'
                        teamid = row['Team_ID']
                        
                        game_path = f'games/2024-25/{gameid}/team_BoxScores.csv'
                        if os.path.exists(game_path):
    ...
    def saveMatchups(self, mode='offense'):
        ...
        #Collects game data
        #for each game go through each matchup
        for game_id in self.game_ids:
            ...
            for attempt in range(retry_count):
                try:
                    ...
                    play_by_play.to_csv(f'games/2024-25/{game_id}/play_by_play.csv')
                    play_by_playv2 = playbyplayv2.PlayByPlayV2(game_id=game_id).play_by_play.get_data_frame()
                    play_by_playv2.to_csv(f'games/2024-25/{game_id}/play_by_playv2.csv')
                    play_by_playv3 = playbyplayv3.PlayByPlayV3(game_id=game_id).play_by_play.get_data_frame()
                    play_by_playv3.to_csv(f'games/2024-25/{game_id}/play_by_playv3.csv')
        ...
        try:
            for index, row in all_matchups_df.iterrows():
                player_name = self.normalize_player_name(row['Player Name'])
                opponent_name = self.normalize_player_name(row['Opponent'])
                

                # Initialize variables to store player and opponent information
                player_info = self.players.get(player_name, {})
                opponent_info = self.players.get(opponent_name, {})
                if not player_info or opponent_info:
                    continue
                    
                # Check if essential information is available, otherwise use default values
                player_height = player_info['bio'].get('HEIGHT', 'N/A')
                player_position = player_info['bio'].get('POSITION', 'N/A')
                player_weight = player_info['bio'].get('WEIGHT', 'N/A')
                player_obpm = player_info['advanced'].get('OBPM', 0)
                player_dbpm = player_info['advanced'].get('DBPM', 0)
                player_team = player_info['bio'].get('TEAM_ABBREVIATION', 'N/A')
                player_wingspan = player_info['bio'].get('WINGSPAN', 'N/A')
                player_speed = player_info['gamelogs'].get('AVG_SPEED', 'N/A')
                average_passes_per_poss = player_info['gamelogs'].get('OFF_POSS_PASSES', 0)
                player_poss_rebchances = player_info['gamelogs'].get('POSS_REB_CHANCES', 'N/A')
      
                
                
                opponent_height = opponent_info['bio'].get('HEIGHT', 'N/A')
                opponent_position = opponent_info['bio'].get('POSITION', 'N/A')
                opponent_weight = opponent_info['bio'].get('WEIGHT', 'N/A')
                opponent_minutes = opponent_info['gamelogs'].get('AVG_MIN', 0)
                opponent_obpm = opponent_info['adv'].get('OBPM', 0)
                opponent_dbpm = opponent_info['adv'].get('DBPM', 0)
                opponent_team = opponent_info.get('TEAM_ABBREVIATION', 'N/A')
                opponent_wingspan = opponent_info['bio'].get('WINGSPAN', 'N/A')
                opponent_speed = opponent_info['gamelogs'].get('AVG_SPEED', 'N/A')
                opponent_poss_rebchances = opponent_info['gamelogs'].get('POSS_REB_CHANCES', 'N/A')
        ...
        for player in self.players:
            ...
            if player_df.empty or opponent_df.empty:
                continue
            ...
            try:
                ...
                player_height = round(float(self.players[player]['bio']['HEIGHT']),2)
                player_weight = round(float(self.players[player]['bio']['WEIGHT']),2)
                position = self.players[player]['bio']['POSITION']
                #add points/game and ast per game
                Avg_Min = self.players[player]['gamelogs']['AVG_MIN']
                Avg_Pts = self.players[player]['gamelogs']['offence']['OFF_AVG_PTS']
                Avg_Ast = self.players[player]['gamelogs']['offence']['OFF_AVG_AST']
                ...
                if mode == 'defense':
                    for index, row in opponent_df.iterrows():
                        ...
                        opponent_name = self.normalize_player_name(opponent_name.replace('Jr',''))
                        # Use opponent's 'AVG_MIN' for defensive dataframe
                        #defense should be average stat/game as well as average stat/min,
                                                        
                        opponent_df.loc[index, 'Avg Min'] = self.players[opponent_name]['gamelogs']['AVG_MIN']
                        opponent_df.loc[index, 'Avg Pts'] = self.players[opponent_name]['gamelogs']['offence'].get('OFF_AVG_PTS','n/a')
                        opponent_df.loc[index, 'Avg Ast'] = self.players[opponent_name]['gamelogs']['offence'].get('OFF_AVG_AST','n/a')
                    ...
                    for index,row in player_df.iterrows():
                        ...
                        player_df.loc[index, 'Defender Avg Min'] = self.players[opponent_name]['gamelogs']['AVG_MIN']
                        player_df.loc[index, 'Defender Avg Pts Against'] = self.players[opponent_name]['gamelogs']['defence'].get('DEF_AVG_PTS', 'n/a')
                        player_df.loc[index, 'Defender Poss Pts Against'] = self.players[opponent_name]['gamelogs']['defence'].get('DEF_POSS_PTS', 'n/a')
                        player_df.loc[index, 'Defender Avg Ast Against'] = self.players[opponent_name]['gamelogs']['defence'].get('DEF_AVG_AST','n/a')

            except ValueError:
                pass
            
            except KeyError as e:
                print(e, 'KeyError - player/opponent and self.players error in SaveMatchups')
    ...
    for filename in os.listdir('teams/games/gamelogs/'):
        ...
        if '._' not in file_path and '_.' not in file_path:
            try:
                ...
                # Ensure 'Opponent' is mapped correctly
                for index, row in team_log.iterrows():
                    opponent_abbrev = row['TEAM_ABBREVIATION']  # Assuming 'TEAM_ABBREVIATION' column is present
                    
                    # Fetch corresponding team metrics using abbreviation
                    team_data = team_metrics[team_metrics['Team'] == opponent_abbrev]
